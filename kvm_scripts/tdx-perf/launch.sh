...
# 2. virt-customize -a ubuntu-24.04-server-cloudimg-amd64.img --install openssh-server
...
#	sed -i 's/^PasswordAuthentication/#PasswordAuthentication/' /etc/ssh/sshd_config.d/60-cloudimg-settings.conf
#	echo "PasswordAuthentication yes" >> /etc/ssh/sshd_config.d/60-cloudimg-settings.conf
#	systemctl restart ssh
# 4(OPT). Edit the config in VM guest(note, need to use bridge mode)
...
#
# 5. netplan apply
# 6. need to extend the disk size by:
#    host: qemu-img resize ubuntu-24.04-server-cloudimg-amd64.img +10G
#    guest: sudo growpart /dev/vda 1
#    guest: sudo resize2fs /dev/vda1
#    guest: df -h
# 7. If you want to mount the qcow on host and change the
#    boot up option in grub, you can do like this:
...
#    change grub.cfg
#    set default="Advanced options for Ubuntu>Ubuntu, with Linux 6.8.0-101-generic"
#    umount mnt
